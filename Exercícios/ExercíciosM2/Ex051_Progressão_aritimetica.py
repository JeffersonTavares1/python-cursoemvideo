...
from banner import apresentação, carregamento, data, hora, cores, frases
...
while True:
    apresentação(ativo_linha1=False, cor_texto1=f"\x1b[7m{choice(cores)}", texto1=" <Ichiro", texto2=f"\x1b[48;5;247m{choice(cores)}{operador}\x1b[m", ativo_linha2=False, ativo_tempo=True, ativador=apresentação_interuptor)   
    ...
    if tela.startswith("S"):
        ...
        valor1 = str(input(f"\x1b[1m{choice(cores)}" if resultado == 0 else f"\x1b[1m{choice(cores)}{resultado}    \x1b[38;5;237m enter pra usar esse valor\r" + f"{choice(cores)}")).strip() or str(resultado)
        ...
        print("\x1b[1m\x1b[38;5;247m" + ("一" * 18) + "\x1b[m")
        op = ""
        while op not  in "0123456TMSVNH" or not bool(op):
